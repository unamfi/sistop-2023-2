...
- Comentarios ::
  ...
  - Respecto  las respuestas que entregan: Recuerden que en
    /Retroalimentación Multinivel/ el administrador no /asigna/
    prioridades, sino que las prioridades /se van determinando
    automáticamente/ por la duración de un proceso en la cola. Todos
    los procesos llegan siendo de máxima prioridad (y, contrario a lo
    que indican, sólo son /degradados/, nunca promovidos).
  - Ya viendo las implementaciones: Las decisiones de planificación se
    toman /a cada momento, basados en la situación existente/. Eso
    significa que en la lotería, a cada quantum, sólo participan en el
    sorteo aquellos procesos /que ya llegaron y no han
    finalizado/. Esto es, ante una ejecución como esta:
    #+begin_src text
	 ...
	 E          5         82
      ...
      -------CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       ...
       - Duración total ⏱️ : 44 -
    #+end_src
    Para cualquier ~t < 5~, sólo se sortea entre los boletos de =A=
    (porque es el único proceso que ha llegado). Para ~5 <= t < 7~ se
  ...
  - Para /Retroalimentación Multinivel/, me parece que la
    implementación está mayomente correcta, pero se detiene en el
    momento en que no hay ningún proceso pendiente. Esto es:
    #+begin_src text
	 ...
	 D               6               8
      ...
      A terminó en el tiempo 7
    #+end_src
    Como los procesos =B= y =D= llegan en =t=8=, y el algoritmo no
    contempla que puede haber /tiempos muertos/ legítimamente, termina
    antes de tiempo, habiendo ejecutado únicamente a uno de los
    procesos.
  - Veo que agregaron la cláusula que sugieren en el texto:
    #+begin_src python
  ...
  - ¡Una disculpa por no haber terminado de calificar esta
    implementación junto con las otras!
