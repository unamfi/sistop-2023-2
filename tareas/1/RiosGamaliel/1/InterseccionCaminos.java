...
public class InterseccionCaminos {
  ...
  public static void main(String... args) {
    ...
    // Carril izquierda (←)
    new Thread(
      new Carril("IZQ", autos.get(0), s[1], s[0], s[2], producerMutex[0])
    ).start();

    // Carril abajo (↓)
    new Thread(
      new Carril("ABJ", autos.get(1), s[0], s[2], s[3], producerMutex[1])
    ).start();

    // Carril derecha (→)
    new Thread(
      new Carril("DER", autos.get(2), s[2], s[3], s[1], producerMutex[2])
    ).start();

    // Carril arriba (↑)
    new Thread(
      new Carril("ARR", autos.get(3), s[3], s[1], s[0], producerMutex[3])
    ).start();
  }
}
